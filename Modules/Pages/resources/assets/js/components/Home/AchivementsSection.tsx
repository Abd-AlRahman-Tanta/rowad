...
const AchivementsSection = ({ achivements, achivementsLabel, achivementsTitle, achivementsImage }: AchivementsProps) => {
  ...
  return (
    <div ref={ref} id='achivements' className='w-full scroll-m-20 px-largeSaveSpace max-desc:px-mobSaveSpace  py-20 relative'>
      <EditableImage start='90%' top='4rem' path='achivementsImage' src={achivementsImage} className={`w-full h-full absolute! top-0 left-0 ${pBg?.className ?? ''}`} style={pBg?.style}>
      ...
      <EditableText path='achivementsTitle' text={achivementsTitle} className={`mb-6 w-fit mx-auto mt-2 ${pTitle?.className ?? ''}`} style={pTitle?.style}>
            ...
            achivements.map((ahv, i) => {
              ...
              return (
                <EditableObject
                  ...
                  start='40%'
                >
                  <div className={`w-fit max-desc:w-full  flex max-desc:flex-col-reverse 
